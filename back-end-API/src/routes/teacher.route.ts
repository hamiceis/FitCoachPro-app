  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!teacher) {
        return res
          .status(401)
          .json({ message: "professor não encontrado, ou não informado" });
      }
      ...
      if (students.length === 0)
        return res
          .status(401)
          .json({ message: "Não foram encontrados registros" });
      ...
      return res
        .status(500)
        .json({ message: "INTERNAL_SERVER_ERRO [GET_professorId_students]" });
    }
  }
);
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (teacher?.id !== user.id) {
        return res
          .status(402)
          .json({ message: "Professor não encontrado ou não autorizado" });
      }
...
routeTeacher.put("/teacher", authLogin, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (teacher.id !== user.id) {
      return res
        .status(403)
        .json({ message: "Você não tem permissão para atualizar" });
    }
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (student.professor) {
        return res
          .status(400)
          .json({ message: "Aluno já associado a um professor" });
      }

      const filterStudent = teacher.students.filter(
      ...
      const dataNewStudent =
        newStudent.students[newStudent.students.length - 1];

      return res
        .status(200)
        .json({
          message: `Aluno ${dataNewStudent.name} cadastrado com sucesso a sua lista`,
        });
    } catch (error) {
    ...
    try {
      ...
      if (teacher.id !== user.id) {
        return res
          .status(403)
          .json({
            message: "Você não tem permissão para atualizar dados desse aluno",
          });
      }
      ...
      if (!student) {
        return res
          .status(401)
          .json({ message: "Student não está na sua lista" });
      }
      ...
      if (body.conditioning_level)
        data.conditioning_level = body.conditioning_level;
      if (body.weekly_Frequency) data.weekly_Frequency = body.weekly_Frequency;
      if (body.goal) data.goal = body.goal;
      if (body.protocol_start_date)
        data.protocol_start_date = dayjs(
          body.protocol_start_date
        ).toISOString();
      if (body.protocol_end_date)
        data.protocol_end_date = dayjs(body.protocol_end_date).toISOString();
      ...
      res
        .status(500)
        .json({ message: "INTERNAL_SERVER_ERROR_PUT_TEACHER_Student" });
    }
  }
);
...
routeTeacher.patch(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: "INTERNAL_SERVER_ERROR_PUT_TEACHER" });
    }
  }
);
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      return res
        .status(200)
        .json({
          message: `Aluno(a) ${name} cadastrado(a) com sucesso a sua lista`,
        });
    } catch (error) {
      console.log("[INTERNAL_SERVER_ERROR_teacher_post]", error);
      return res.status(500).json({ message: "Internal Server Error" });
    ...
    try {
      ...
      if (student.professorId !== teacher.id) {
        return res
          .status(404)
          .json({
            message: "Professor não tem autorização para remover esse aluno",
          });
      }
      ...
      res
        .status(200)
        .json({ message: "Aluno removido da lista do professor com sucesso" });
    } catch (error) {
